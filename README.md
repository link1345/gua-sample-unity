...
UI automation uses the Gua v0.15.0 Unity UPM package. At runtime, it opens a Gua bridge at `ws://127.0.0.1:8765` by default and automatically exposes the standard uGUI tree. Set the `GUA_BRIDGE_PORT` environment variable to use a different port.
...
The official UPM package v0.15.0, including the managed assemblies and Windows x64 native libraries, is pinned in `Packages/com.link1345.gua`. No adjacent Gua source repository or separate package download is required.
...
This sample targets Windows x64 and Mono, matching the initial Unity support in Gua v0.15.0. IL2CPP and non-Windows Players are outside the sample's validated scope.
